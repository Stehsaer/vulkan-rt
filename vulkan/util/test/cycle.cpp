...
TEST_CASE("Construction")
{
	...
	SUBCASE("Move only type")
	{
		...
		CHECK_EQ(cycle.current().time_moved, 1);
	}
}
...
TEST_CASE("Cycle")
{
	...
	// [3, 1, 2]
	CHECK_EQ(cycle.current(), 2);
	CHECK_EQ(cycle.prev(), 3);

	cycle.cycle();

	// [2, 3, 1]
	CHECK_EQ(cycle.current(), 1);
	CHECK_EQ(cycle.prev(), 2);
}
...
TEST_CASE("Iterate")
{
	SUBCASE("Iterate through items")
	{
		...
		CHECK_EQ(iterated_items[0], 1);
		CHECK_EQ(iterated_items[1], 2);
		CHECK_EQ(iterated_items[2], 3);
	}
	...
	SUBCASE("Iterate through pairs")
	{
		...
		CHECK_EQ(iterated_pairs[0].first, 2);
		CHECK_EQ(iterated_pairs[0].second, 1);

		CHECK_EQ(iterated_pairs[1].first, 3);
		CHECK_EQ(iterated_pairs[1].second, 2);

		CHECK_EQ(iterated_pairs[2].first, 1);
		CHECK_EQ(iterated_pairs[2].second, 3);
	}
}
